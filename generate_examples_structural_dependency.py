...
repo_path= "gigant/graphlongt5-structural-dependency-0408" #"gigant/longt5-global-3epoch" #"gigant/graph-t5-global-window-8k-longt5local" # ==> my checkpoint
...
def preprocess_function(examples):
    ...
    model_inputs["graph_dependency"] = [get_dependency_graph(dep_graph) for dep_graph in examples["dependency_graph"]]
...
def data_loader(rng, dataset, batch_size, shuffle: bool = False, drop_last=False):
    ...
    for idx in batch_idx:
        ...
        graph_batch_dependency = {
            "receivers_dependency": np.stack([graph["receivers_dependency"] for graph in graph_batch_dependency]).astype(np.int16),
            "senders_dependency": np.stack([graph["senders_dependency"] for graph in graph_batch_dependency]).astype(np.int16),
            "edge_labels_dependency": np.stack([graph["edge_labels_dependency"] for graph in graph_batch_dependency]).astype(np.int16)
            }
...
for batch, label in tqdm(test_loader):
    ...
    graphs_dependency = graph_from_path(model.params, graph_dependency, {}, {}, layer_wise=False)
    
    params= {"params": model.params, "graph": graph_structural, "graph_dependency": graph_dependency}
...
with open('predictions_strdep.txt', 'w') as fp:
    for line in predictions:
        # write each item on a new line
        fp.write(line + "\n")
# open file in write mode
with open('references_strdep.txt', 'w') as fp:
