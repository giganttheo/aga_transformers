...
def data_loader(rng: jax.random.PRNGKey, dataset: Dataset, model, batch_size: int, shuffle: bool = False, drop_last=True):
    ...
    for idx in batch_idx:
        ...
        graph_batch = {
            ...
            "mask_local": np.stack([graph["mask_local"][0].swapaxes(0, 1) for graph in graph_batch]).astype("bool"),
            "mask_global": np.stack([graph["mask_global"][0].swapaxes(0, 1) for graph in graph_batch]).astype("bool"),
