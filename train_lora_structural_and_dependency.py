...
from aga_transformers.models.t5.t5 import load_t5, load_efficient_t5, load_augmented_t5, load_slide_and_dependency_t5
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        tokenizer, model, graph, graph_ar = load_slide_and_dependency_t5(repo_path=model_args.model_name_or_path, dtype="bfloat16", attention_kwargs=attention_kwargs, from_longt5_local=True, layer_wise=False)
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(inputs)):
            ...
            receivers_dep = jax.lax.dynamic_update_slice(receivers_dep, jnp.array([ (num_slides[i] + r) for r,s,gm in zip(dep_graph["receivers"], dep_graph["senders"], dep_graph["graph_mask"]) if (num_slides[i] + r) < seq_length and (num_slides[i] + s) < seq_length and gm], dtype=jnp.uint16), (0,))
            senders_dep = jnp.zeros((max_graph_len), dtype=jnp.uint16)
            senders_dep = jax.lax.dynamic_update_slice(senders_dep, jnp.array([(num_slides[i] + s) for r,s,gm in zip(dep_graph["receivers"], dep_graph["senders"], dep_graph["graph_mask"]) if (num_slides[i] + r) < seq_length and (num_slides[i] + s) < seq_length and gm], dtype=jnp.uint16), (0,))
        ...
        model_inputs["graph_dependency"] = graphs
    ...
    if training_args.do_train:
        ...
        if loading_ds_from_disk:
            ...
            preprocessed_datasets = load_from_disk("./preprocessed_datasets/slide_and_dependency")
    ...
    def train_step(state, batch):
        ...
        def compute_loss(params):
            ...
            receivers_dependency = batch.pop("receivers_dependency")
            senders_dependency = batch.pop("senders_dependency")
            edge_labels_dependency = batch.pop("edge_labels_dependency")
            graph_dependency = {"receivers": receivers_dependency, "senders": senders_dependency, "edge_labels": edge_labels_dependency}
            graphs_dependency = graph_from_path(state.params, graph_dependency, {}, {}, layer_wise=False)

            loss, _ = loss_fn_(model=state.apply_fn, params=params, graph=graphs, graph_dependency=graphs_dependency, dropout_rng=dropout_rng, **batch)
            return loss, None
    ...
    def eval_step(params, batch):
        ...
        loss, _ = loss_fn(model=state.apply_fn, params=params, graph=graphs, graph_dependency=graphs_dependency, train=False, **batch)
