...
def tie_relative_pos_bias(module_class, repo_path):
  ...
  first_block_relative_attention_bias = {k: (k,'block','0','layer','0','SelfAttention','relative_attention_bias', 'embedding') for k in ['encoder', 'decoder']}
  other_blocks_relative_attention_bias = {k: [(k,'block', str(b),'layer','0','SelfAttention','relative_attention_bias', 'embedding') for b in range(1, n_blocks)] for k in ['encoder', 'decoder']}
  rules = {source:
