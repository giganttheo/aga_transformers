...
class FlaxT5EfficientBlockGraphSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        position_bias_global = einops.repeat(self.compute_bias(query_length=n_global_tokens, key_length=seq_length), "h n d -> b h n d", b=batch_size)
        ...
        if self.has_graph_edge_bias and not no_graph:
            ...
            position_bias_local = jax.lax.select(einops.repeat(edge_bias_local, "b n k d -> b h n k d", h=self.n_heads)<0, position_bias_local, jnp.zeros(position_bias_local.shape).astype(self.dtype))
            ...
            # no position bias if there is an edge bias
            position_bias_global = jax.lax.select(einops.repeat(edge_bias_global, "b n d -> b h n d", h=self.n_heads)<0, position_bias_global, jnp.zeros(position_bias_global.shape).astype(self.dtype))
            ...
            _ = self.graph_edge_bias(jnp.zeros(position_bias_local.shape, dtype=jnp.uint16))
