...
def _concatenate_3_blocks_and_global_with_slides(x: jnp.ndarray, x_global: jnp.ndarray, slide_start_for_blocks: jnp.ndarray, n_slides_: int, n_document_tokens: int, doc_tokens_start:int, block_axis: int, sequence_axis: int, pad_value: int = 0) -> jnp.ndarray:
    ...
    @partial(jax.vmap, in_axes=(1, None, None), out_axes=1) #blocks
...
class FlaxT5EfficientBlockGraphSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        position_bias_local = self._create_block_position_bias(block_len, n_global_tokens, num_blocks, n_document_tokens, n_slides_total)
