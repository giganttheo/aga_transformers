    ...
    def __call__(
        ...
        if self.has_variable("graph", "receivers"):
            ...
            if self.has_graph_edge_bias:
                ...
                edge_bias_local = jax.lax.select(edge_bias_local[..., None]>=0, self.graph_edge_bias(edge_bias_local), jnp.zeros(tuple(edge_bias_local.shape) + (1,)).astype(self.dtype))
                ...
                edge_bias_global = jax.lax.select(edge_bias_global[..., None]>=0, self.graph_edge_bias(edge_bias_global), jnp.zeros(tuple(edge_bias_global.shape) + (1,)).astype(self.dtype))
