    ...
    def __call__(
        ...
        if self.has_variable("graph", "receivers"):
            ...
            if graph_mask is not None:
                jax.debug.print("s: pos_bias: {position_bias} for edges {zip(senders, receivers)}", position_bias=position_bias[0, 0, :10], senders=senders[0, :10], receivers=receivers[0, :10])
                jax.debug.print("e: pos_bias: {position_bias} for edges {zip(senders, receivers)}", position_bias=position_bias[0, 0, -10:], senders=senders[0, -10:], receivers=receivers[0, -10:])
