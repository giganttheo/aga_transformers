...
# Structure-Augmented Functional Interpolation for Relative Positions

# TO TEST:
# latency test vs embedding
# parameters count vs embedding
# pretrain by approximating the embeddings
...
class FIRE(nn.Module):
    ...
    hidden_size: int
    eps: float = 1e-6
    c_init: Callable[..., np.ndarray] = jax.nn.initializers.constant(0.01)
    L_init: Callable[..., np.ndarray] = jax.nn.initializers.constant(512)
    dtype: jnp.dtype = jnp.float32  # the dtype of the computation

    def setup(self):
        l0_init_std = self.config.initializer_factor * (self.hidden_size**-0.5)
        l1_init_std = self.config.initializer_factor * (self.config.num_heads**-0.5)
        self.c = self.param("c", self.c_init, (1,))
        self.L = self.param("L", self.L_init, (1,))
        self.mlp = [
            nn.Dense(
                self.hidden_size,
                use_bias=False,
                kernel_init=jax.nn.initializers.normal(l0_init_std),
                dtype=self.dtype,
            ),
            nn.Dense(
                self.config.num_heads,
                use_bias=False,
                kernel_init=jax.nn.initializers.normal(l1_init_std),
                dtype=self.dtype,
            ),
        ]
    ...
    def _compute_bias(self, memory_position, context_position):
        ...
        pos_normalizer = jnp.maximum(memory_position, threshold)
        relative_position = jnp.log(jnp.abs(self.c * relative_position) + 1)
    ...
    def __call__(self, pos_q, pos_k):
        """
        pos_q: (..., seq_len,), is the (learnt) positions of q
        pos_k: (..., seq_len,) is the (learnt) positions of k

        Outputs:

        bias: (..., seq_len, num_heads)
        """

        return self._compute_bias(pos_q, pos_k)

# class SAFIRE(nn.Module):
#     config: T5Config
#     hidden_size: int
#     eps: float = 1e-6
#     c_init: Callable[..., np.ndarray] = jax.nn.initializers.constant(0.01)
...
#     @jnp.vmap
#     def __call__(self, prior, pos_q, pos_k, delta_t):
#         """
#         prior: (..., seq_len, prior_dim) is a vector containing the prior information
#         pos_q: (..., seq_len,), is the (learnt) positions of q
#         pos_k: (..., seq_len,) is the (learnt) positions of k
#         delta_t: (..., seq_len,) is the normalized time difference between tokens
...
#         rel_distance = pos_q[:, None] - pos_k[None, :] #TODO
#         threshold = jnp.abs(self.L)
#         pos_normalizer = jnp.maximum(pos_q, threshold)
#         rel_distance = jnp.log(jnp.abs(self.c * rel_distance) + 1)
#         pos_normalizer = jnp.log(jnp.abs(self.c * pos_normalizer) + 1) + self.eps

#         normalized_distance = rel_distance / pos_normalizer
#         x = jnp.concatenate([normalized_distance, delta_t, prior])
#         hidden_gelu = self.act(self.mlp[0](x))
#         bias = self.act(self.mlp[1](hidden_gelu))
#         return bias
...
class FlaxT5EfficientBlockGraphSelfAttention(nn.Module):
    ...
    def setup(self):
        self.n_global_tokens = 2
        self.block_len = 254//2 + 1 #TODO: add in config (radius + 1)
    ...
    def __call__(
        ...
        block_len=self.block_len #254+1  #TODO: add in config (radius + 1)
        n_global_tokens = self.n_global_tokens #TODO: add in config
