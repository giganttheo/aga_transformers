...
class LongformerAttentionPattern(AttentionPattern):
  def __init__(self, seq_len_q, seq_len_kv, window_size, block_size=1, sentence_tokens=[], dilation=None, n_heads=4, batch_size = 2):
    super().__init__()
    ...
    for head in range(n_heads):
      ...
      for i in seq_kv:
        for j in seq_q:
           ...
           block = abs((i // block_size ) - (j // block_size )) <= window_size
           if i in global_tokens or j in global_tokens or (block and j in window):
...
def create_dilated_window_attn_patterns(model, max_source_length, max_target_length, n_heads, batch_size, window_size=5, block_size=1, autoregressive=True):
