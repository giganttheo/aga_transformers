...
from aga_transformers.models.t5.t5 import load_t5, load_efficient_t5, load_fire_t5
...
vqa_name_mapping = {
    "gigant/pdfvqa": ("texts", "images", "questions", "answers"),
}
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        tokenizer, model, graph, graph_ar = load_fire_t5(repo_path=model_args.model_name_or_path, dtype="bfloat16", attention_kwargs=attention_kwargs, layer_wise=False)
    ...
    dataset_columns = vqa_name_mapping.get(data_args.dataset_name, None)
    if data_args.text_column is None:
        if dataset_columns is None:
            print("\n\n\n==================/!\==================\n")
            print(f"Text column is not set, defaulting to {column_names[0]}")
        text_column = dataset_columns[0] if dataset_columns is not None else column_names[0]
        img_column = dataset_columns[1] if dataset_columns is not None else "images"
        question_column = dataset_columns[2] if dataset_columns is not None else "questions"
        answer_column = dataset_columns[3] if dataset_columns is not None else "answers"
    else:
    ...
    def relative_center_distances(bbox1, bbox2, height, width):
        x1,y1,w1,h1 = bbox1
        x2,y2,w2,h2 = bbox2
        cx1 = x1 + w1/2
        cy1 = y1 + h1/2
    ...
    def preprocess_function(examples):
        ...
        targets = [answers[0] for answers in examples[answer_column]]
        # inputs = [f"question: {question[0]}, context: {inp}" for inp, question in zip(inputs, questions)]
        ...
        model_inputs = tokenizer(
            inputs,
            max_length=data_args.max_source_length, padding="max_length", truncation=True, return_tensors="np"
        )
        ...
        for i in range(len(inputs)):
            ...
            offset_tokens_between_blocks = len(tokenizer(
                            f"\n",
                            max_length=data_args.max_source_length,
                            padding="do_not_pad",
                            truncation=True,
            ...
            for group_id, t in enumerate(example['texts']):
                if not t is None:
                    ...
                    box_ids_.extend([group_id]*len(tok_t['input_ids'][0]))
                    total_nodes += len(tok_t['input_ids'][0])
                else:
            ...
            def edge_info(i, j):
                #compute the distances between node i and j
                ...
                if box_id1 is not None and box_id2 is not None:
                    hd = hdist[box_id1, box_id2]
                    vd = vdist[box_id1, box_id2]
                else:
                    # not applicable
                    hd = 0
                    vd = 0

                return tok_dist, hd, vd
    ...
    def save_state(state, train_metrics, pred_metrics):
        ...
        # with open(CKPT_DIR + "train_metric_state.pickle", "wb") as outfile:
        #     pickle.dump(train_metrics, outfile, protocol=pickle.HIGHEST_PROTOCOL)
        # with open(CKPT_DIR + "pred_metric_state.pickle", "wb") as outfile:
        #     pickle.dump(pred_metrics, outfile, protocol=pickle.HIGHEST_PROTOCOL)
    
    def load_state():
        with open(CKPT_DIR + "opt_state.pickle", "rb") as file:
            state_ = pickle.load(file)
        # with open(CKPT_DIR + "train_metric_state.pickle", "rb") as file:
        #     train_metrics = pickle.load(file)
        # with open(CKPT_DIR + "pred_metric_state.pickle", "rb") as file:
        #     pred_metrics = pickle.load(file)
        return state_#, train_metrics, pred_metrics

    if training_args.resume_from_checkpoint:
        loaded_state = load_state()
        state = state.replace(**loaded_state)
        print("\n\n\n")
